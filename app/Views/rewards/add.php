            ...
            <div class="card">
                <div class="col-md-6">
                    <div class="card card-primary">
                        ...
                        <form method="post" action="{{ __WEB__ROOT . '/reward/check/create'}}">
                            <div class="card-body">
                                <div class="form-group">
                                        <label for="inputName">Tên giải thưởng </label>
                                        <input type="text" class="form-control" value="{{oldData('reward_name')}}" name="reward_name" placeholder="Nhập vào tên giải thưởng">
                                        @php echo formError('reward_name', '<span style="color:red;">','</span>') @endphp                    
                                    </div>

                                    <div class="form-group">
                                        <label for="inputAmount">Tiền thưởng </label>
                                        <input type="text" class="form-control" value="{{oldData('amount')}}" name="amount"  placeholder="Nhập số tiền thưởng">
                                        @php echo formError('amount', '<span style="color:red;">','</span>') @endphp
                                    </div>

                                    <div class="form-group">
                                        <label for="inputNote">Ghi chú </label>
                                        <input type="note" class="form-control" value="{{oldData('note')}}" name="note" id="inputNote" placeholder="Nhập vào ghi chú">
                                    </div>
                                </div>
                            </div>
